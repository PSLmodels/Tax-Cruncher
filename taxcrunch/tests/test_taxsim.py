...
def test_a18_validation():
    taxcrunch_in = os.path.join(
        CURRENT_PATH, "taxsim_validation/taxcrunch_in_a18.csv"
    )
    crunch = mcr.Batch(taxcrunch_in)
    table_a18 = crunch.create_table()

    taxsim_out = os.path.join(
        CURRENT_PATH, "taxsim_validation/taxsim_out_a18.csv"
    )
    ...
    assert np.allclose(
        table_a18["Individual Income Tax"], taxsim_df["fiitax"], atol=0.01
    )
...
def test_c18_validation():
    taxcrunch_in = os.path.join(
        CURRENT_PATH, "taxsim_validation/taxcrunch_in_c18.csv"
    )
    crunch = mcr.Batch(taxcrunch_in)
    emulation = os.path.join(
        CURRENT_PATH, "taxsim_validation/taxsim_emulation.json"
    )
    table_c18 = crunch.create_table(reform_file=emulation)

    taxsim_out = os.path.join(
        CURRENT_PATH, "taxsim_validation/taxsim_out_c18.csv"
    )
    taxsim_df = pd.read_csv(taxsim_out)
    ...
    assert np.allclose(
        table_c18["Individual Income Tax"], taxsim_df["fiitax"], atol=0.01
    )
